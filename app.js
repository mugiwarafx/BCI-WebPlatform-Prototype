...
var nodes = [
  {
    ...
    name: "Datos 1",
    type: "Data JSON",
    description: "Descripción del Nodo 1",
  },
  {
    id: 2,
    name: "Filtros 2",
    type: "Filtro Pasa Banda",
    description: "Descripción del Nodo 2",
  },
];
...
function addNode() {
  ...
  const sourceNodeId = parseInt(
    document.getElementById("sourceNodeSelector").value,
    10
  );
...
function updateNodeList() {
  ...
  nodes.forEach((node) => {
    ...
    deleteButton.innerText = `Eliminar Nodo ${node.name}`;
...
function addLink() {
  ...
  const targetNodeId = parseInt(
    document.getElementById("targetNodeSelector").value,
    10
  );
